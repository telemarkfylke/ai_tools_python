...
for filename in os.listdir(sourcedir):
    if filename.endswith('.m4v') or filename.endswith('.mp4'):
        ...
        ffmpeg.input(input_file).output(audio_file, acodec='libmp3lame', format='mp3', t=600).run()
        print(f'Hentet ut lyd fra {input_file} og lagret i {audio_file}')

        # Laster inn KI-modellen fra Huggingface
        asr = pipeline("automatic-speech-recognition", "NbAiLab/nb-whisper-small", device='cpu')
